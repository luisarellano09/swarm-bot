...
RC_e ExtHandler(Request* request){
    ...
    switch(request->regId){
        ...
        case COM_REQUEST_REG_ID_e::SETUP_MOTOR_LEFT_OFFSET_W: {
            manager->m_motor01->SetOffset((double)request->data / 100.0);
            manager->m_nodeESP32->UpdateRegister(COM_REQUEST_REG_ID_e::SETUP_MOTOR_LEFT_OFFSET_R, manager->m_motor01->GetOffset() * 100);
            Log.infoln("[NodeHandler::SETUP_MOTOR_LEFT_OFFSET_W] Motor left offset setted: %D", manager->m_motor01->GetOffset());
            break;
        }
        
        case COM_REQUEST_REG_ID_e::SETUP_MOTOR_LEFT_DIRECTION_W: {
            manager->m_motor01->SetDirection((bool)request->data);
            manager->m_nodeESP32->UpdateRegister(COM_REQUEST_REG_ID_e::SETUP_MOTOR_LEFT_DIRECTION_R, manager->m_motor01->GetDirection());
            Log.infoln("[NodeHandler::SETUP_MOTOR_LEFT_DIRECTION_W] Motor left direction: %T", manager->m_motor01->GetDirection());
            break;
        }
          
        case COM_REQUEST_REG_ID_e::SETUP_MOTOR_RIGHT_OFFSET_W: {
            manager->m_motor02->SetOffset((double)request->data / 100.0);
            manager->m_nodeESP32->UpdateRegister(COM_REQUEST_REG_ID_e::SETUP_MOTOR_RIGHT_OFFSET_R, manager->m_motor02->GetOffset() * 100);
            Log.infoln("[NodeHandler::SETUP_MOTOR_RIGHT_OFFSET_W] Motor right offset setted: %D", manager->m_motor02->GetOffset());
            break;
        }
        
        case COM_REQUEST_REG_ID_e::SETUP_MOTOR_RIGHT_DIRECTION_W: {
            manager->m_motor02->SetDirection((bool)request->data);
            manager->m_nodeESP32->UpdateRegister(COM_REQUEST_REG_ID_e::SETUP_MOTOR_RIGHT_DIRECTION_R, manager->m_motor02->GetDirection());
            Log.infoln("[NodeHandler::SETUP_MOTOR_RIGHT_DIRECTION_W] Motor right direction: %T", manager->m_motor02->GetDirection());
            break;
        }
       
        case COM_REQUEST_REG_ID_e::SETUP_ENCODER_LEFT_DIRECTION_W: {
            manager->m_encoder01->SetDirection((bool)request->data);
            manager->m_nodeESP32->UpdateRegister(COM_REQUEST_REG_ID_e::SETUP_ENCODER_LEFT_DIRECTION_R, manager->m_encoder01->GetDirection());
            Log.infoln("[NodeHandler::SETUP_ENCODER_LEFT_DIRECTION_W] Encoder left direction: %T", manager->m_encoder01->GetDirection());
            break;
        }

        case COM_REQUEST_REG_ID_e::SETUP_ENCODER_RIGHT_DIRECTION_W: {
            manager->m_encoder02->SetDirection((bool)request->data);
            manager->m_nodeESP32->UpdateRegister(COM_REQUEST_REG_ID_e::SETUP_ENCODER_RIGHT_DIRECTION_R, manager->m_encoder02->GetDirection());
            Log.infoln("[NodeHandler::SETUP_ENCODER_RIGHT_DIRECTION_W] Encoder right direction: %T", manager->m_encoder02->GetDirection());
            break;
        }
        ...
        case COM_REQUEST_REG_ID_e::LIVE_MOTOR_LEFT_SPEED_W: {
            manager->m_motor01->SetSpeed((double)request->data / 100.0);
            Log.infoln("[NodeHandler::LIVE_MOTOR_LEFT_SPEED_W] Motor left speed setted: %D", manager->m_motor01->GetSpeed());
            break;
        }

        case COM_REQUEST_REG_ID_e::LIVE_MOTOR_RIGHT_SPEED_W: {
            manager->m_motor02->SetSpeed((double)request->data / 100.0);
            Log.infoln("[NodeHandler::LIVE_MOTOR_RIGHT_SPEED_W] Motor right speed setted: %D", manager->m_motor02->GetSpeed());
            break;
        }
...
RC_e UpdateRegistersRT(){
    ...
    if (manager->m_nodeESP32->GetError() == false) {
        ...
        // manager->m_nodeESP32->UpdateRegister(COM_REQUEST_REG_ID_e::LIVE_ENCODER_RIGHT_COUNT_R, manager->m_encoder02->GetCount());
