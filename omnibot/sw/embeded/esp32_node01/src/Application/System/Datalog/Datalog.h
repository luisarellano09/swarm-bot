...
void ActivateDataset(Datasets_e select){
    ...
    Log.traceln("[Datalog::ActivateDataset] Dataset activated: %s", datalogDatasets[select].Name.c_str());
}
...
void DeactivateDataset(Datasets_e select){
    ...
    Log.traceln("[Datalog::DeactivateDataset] Dataset deactivated: %s", datalogDatasets[select].Name.c_str());
}
...
void DatalogDatasetMotor(){
    Serial.printf("ML,%.2f, MR,%.2f, ", manager->m_motor01->GetSpeed(), manager->m_motor02->GetSpeed());
}
